...
def dag_erohina():
    ...
    def df_os(df_merged):
        df_os = df_merged.groupby(['os', 'event_date'])\
    ...
    def df_gender(df_merged):
        df_gender = df_merged.groupby(['gender', 'event_date'])\
        ['views','likes','messages_received','messages_sent','users_received','users_sent'].sum().reset_index()
    ...
    def df_age(df_merged):
        df_age = df_merged.groupby(['age', 'event_date'])\
        ['views','likes','messages_received','messages_sent','users_received','users_sent'].sum().reset_index()
    ...
    def load(df_gender, df_os, df_age):
        ...
        create_table = """
                    CREATE TABLE IF NOT EXISTS test.m_erohina(
                        dimension String,
                        dimension_value String,
                        event_date String,
                        views Int32,
        ...
        ph.to_clickhouse(df = df_total, table = 'm_erohina', connection=connect_test, index=False)
        
    # собственно шаги DAG, каждое из которых - отдельная task     
    df_feed = extract_actions()
    df_mess = extract_message()
    df_merged = transform(df_feed,df_mess)
